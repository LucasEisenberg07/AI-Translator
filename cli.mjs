        ...
        for (const language of endingLanguages) {
            ...
            for (const phrase of phrases) {
                languageTranslations.set(
                    phrase,
                    await translate(phrase, language, contextMap.get(language), wrongAnswers.get(language), startingLanguage, regenerate, api)
                );
            }
...
async function regenerateTranslations() {
    rl.question('Which languages do you want to regenerate (all, specific language, or none): ', async (answer) => {
        if (answer.toLowerCase() === "all") {
            ...
            await regenerateForLanguage(answer);
            regenerateTranslations();
...
async function regenerateForLanguage(language) {
    ...
    try {
        for (const phrase of languageTranslations.keys()) {
            const newTranslation = await translate(
                phrase,
                formattedLanguage,
                contextMap.get(formattedLanguage),
                wrongAnswers.get(formattedLanguage),
                startingLanguage,
                regenerate,
                await createApi(client, model)
            );
